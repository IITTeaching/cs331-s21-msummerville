...
class ArrayList:
    ...
    def __add__(self, other):
        ...
        ### BEGIN SOLUTION
        savedlen = self.len
        newlst = ConstrainedList((self.len)+other.len)
        for j in range(0,self.len):
            newlst[j] = self.data[j]
        self.data = newlst
        k = 0
        for i in range(savedlen,len(self.data)):
                self.data[i] = other.data[k]
                k +=1
        self.len = savedlen + other.len
        return self
        ### END SOLUTION
